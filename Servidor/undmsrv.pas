...
begin
  try
    try
      ...
      TRDWAuthOptionBasic(RESTServicePooler.AuthenticationOptions.OptionParams).Username := 'testserver';
      TRDWAuthOptionBasic(RESTServicePooler.AuthenticationOptions.OptionParams).Password := 'testserver';
...
{$IFDEF DEBUG}
      on e: exception do
        WriteLn('Erro:' + inttostr(LPort) + #13 + 'Ocorreu o seguinte erro: ' + e.message);
{$ENDIF}
    end;
  finally
    ArqIni.Free;
