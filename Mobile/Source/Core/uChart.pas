...
type
  TRectangleHelper = class helper for TRectangle
    ...
    procedure ChartCalendario(Data: TDate; Valores: String; Color: TAlphaColor = TAlphaColors.White; Estilo: Integer = 1); overload;
...
type
  ...
  private
    ...
    procedure Serie(Comp: TVertScrollBox; Por1: Integer; Texto: String); overload;
    procedure Serie(Comp: TVertScrollBox; Texto, Texto2: String; Tipo: Integer = 1); overload;
    procedure Serie(Comp: THorzScrollBox; Por1: Integer; Texto: String); overload;
  ...
  public
    ...
    procedure CardMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
    procedure CardDragOver(Sender: TObject; const Data: TDragObject; const Point: TPointF; var Operation: TDragOperation);
    procedure CardDragDrop(Sender: TObject; const Data: TDragObject; const Point: TPointF);
...
begin
  ...
  if Tipo = vertical then
  begin
    Serie(Vert, int1, Hint);
  end
  else if Tipo = horizontal then
  begin
    Serie(horz, int1, Hint);
  end
  else if Tipo = circular then
  begin
...
procedure TGrafico.CardDragDrop(Sender: TObject; const Data: TDragObject; const Point: TPointF);
begin
...
procedure TGrafico.CardDragOver(Sender: TObject; const Data: TDragObject; const Point: TPointF; var Operation: TDragOperation);
begin
  Operation := TDragOperation.Copy;

end;

procedure TGrafico.CardMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Single);
var
...
procedure TGrafico.Serie(Comp: TVertScrollBox; Por1: Integer; Texto: String);
begin
  ...
  // sTitulo.TextSettings.Font.Family := 'Roboto';
...
begin
  ...
  // Rectangle1.ChartBarHorizontal('Exemplo de Gráfico Delphi Creative',
  // '{Janeiro:8:Teste;20;50}{Fevereiro:50:teste;10;70}','');
  ...
  begin
    ...
    for i       := 0 to TRegEx.Matches(Rotulos, '}').Count - 1 do
    begin
      ...
      rFundo.ChartBarHorizontal(tit.Replace('{', ''), arr_rotulos[i].Replace(tit + ':', '').Replace(tit + '}', ''), TAlphaColors.Red)
  ...
  begin
    ...
    tit       := arr_valor[0];
    Self.ChartBarHorizontal(Titulo, Rotulos.Replace(tit + ':', '').Replace(tit + '}', ''), TAlphaColors.Red)
  end;


  // Rectangle1.ChartBarHorizontal('Exemplo de Gráfico Delphi Creative', '15:Janeiro;50;90;35;10;45;20',ComboColorBox1.Color);

  { Janeiro:150 pagar rece



    if TRegEx.Matches(Rotulos,';').Count <> TRegEx.Matches(Valores,';').Count then begin
...
procedure TRectangleHelper.ChartBarHorizontal(Titulo, Valores: String; Cor: TAlphaColor);
var
  g:                     TGrafico;
  i:                     Integer;
  arr_series, arr_valor: TArray<String>;
...
procedure TRectangleHelper.ChartBarVertical(Titulo, Valores: String; Cor: TAlphaColor);
var
  g:                     TGrafico;
  i:                     Integer;
  arr_series, arr_valor: TArray<String>;
...
procedure TRectangleHelper.ChartCalendario(Data: TDate; Valores: String; Color: TAlphaColor = TAlphaColors.White; Estilo: Integer = 1);
var
  g:                     TGrafico;
  i, J, A, M:            Integer;
  rDia:                  TRectangle;
...
begin
  ...
  for i := 1 to (MonthDays[IsLeapYear(A), M]) do
  begin
    ...
    // rDia.Fill.Color := TAlphaColors.White;
    rDia.Stroke.Color := rDia.Fill.Color;
    // rDia.Fill.Color := TAlphaColors.White;
    rDia.Position.X := (dayofweek(strtodate(Inttostr(i) + '/' + Inttostr(M) + '/' + Inttostr(A))) - 1) * rDia.Width;
    rDia.Position.Y := Y;

    // if dayofweek(strtodate( Inttostr(i) + '/'+ Inttostr(M) +'/'+ inttoStr(A) )) = 1 then
    // rDia.Fill.Color := g.Cor;

    if dayofweek(strtodate(Inttostr(i) + '/' + Inttostr(M) + '/' + Inttostr(A))) = 7 then
    begin
      Y := Y + rDia.Height;
      // rDia.Fill.Color := g.Cor;
    end;
    ...
    // tDia.Hint := 'Ver como fica'#13+Inttostr(i);
    tDia.ShowHint := True;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for J := 1 to TRegEx.Matches(arr_dias[i], ':').Count do
        begin
...
procedure TRectangleHelper.ChartCircular(Titulo: String; Valor: Integer; Cor: TAlphaColor);
var
  g: TGrafico;
begin
  g        := TGrafico.Create(Self, circular);
