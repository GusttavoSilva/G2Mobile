...
class procedure Show( const msg: string);
...
class procedure TLoading.Show( const msg: string);
var
  FService: IFMXVirtualKeyboardService;
begin
  // Panel de fundo opaco...
  Fundo             := TRectangle.Create(FrmMain);
  Fundo.Opacity     := 0;
//  Fundo.Parent      := Frm;
Fundo.Parent      := FrmMain;
  Fundo.Visible     := true;
  ...
  Layout         := TLayout.Create(FrmMain);
  Layout.Opacity := 0;
//  Layout.Parent  := Frm;
  Layout.Parent  := FrmMain;
  Layout.Visible := true;
  ...
  // Animacao...
  Animacao               := TFloatAnimation.Create(FrmMain);
