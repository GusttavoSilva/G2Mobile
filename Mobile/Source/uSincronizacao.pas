...
uses
  ...
  uFraAnimacao,
  FMX.Edit,
  uClasseResumoVendedorItens,
  System.DateUtils,
  uClasseResumoVendedorClientes,
  Loading,
  uClasseProdutos,
  uClasseCamaraFria,
  uClasseClientes,
    ...
    begin
      try
        ...
        try
          DataSet := TFDMemTable.Create(self);
          AguardeChangeMessage('Fotos...');
          TProdutos.new.LimpaTabelaProdFoto.BuscaFotoProdServidor(DataSet).PopulaFotosProdSqLite(DataSet);
        finally
          FreeAndNil(DataSet);
        end;
      finally
...
begin
  ...
  try
    ...
    TExportPedido.new.DeleteItemAtualizado.VerificaPedidosSincronizadosSqlite.BuscaInfoAtualizadaServidor(DataSet)
...
begin
  DmDados.ConexaoInterna.StartTransaction;
  try
    TCamaraFria.new.LimpaTabelaCamaraFria.BuscaCamaraFriaServidor(DataSet).PopulaCamaraFriaSqLite(DataSet);
    DmDados.ConexaoInterna.Commit;
...
begin
  ...
  try
    ...
    TClientes.new.LimpaTabelaClientes.BuscaClientesServidor(codUser, CodVend, DataSet).PopulaClientesSqLite(DataSet);
    DmDados.ConexaoInterna.Commit;
...
begin
  DmDados.ConexaoInterna.StartTransaction;
  try
    TFormaPagamento.new.LimpaTabelaPagamentoPessoa.BuscaPagamentoPessoaServidor(DataSet)
...
begin
  DmDados.ConexaoInterna.StartTransaction;
  try
    TProdutos.new.LimpaTabelaProd.BuscaProdServidor(DataSet).PopulaProdSqLite(DataSet);
...
begin
  DmDados.ConexaoInterna.StartTransaction;
  try
    TResumoVendedorItens.new.LimpaTabelas.BuscaProdVendidosServidor(StartOfTheMonth(Date), EndOfTheMonth(Date), CodVend,
      DataSet).PopulaProdVendidoSqLite(DataSet);

    TResumoVendedorClientes.new.LimpaTabelas.BuscaClienteVendidosServidor(StartOfTheMonth(Date), EndOfTheMonth(Date),
      CodVend, DataSet).PopulaClienteVendidoSqLite(DataSet);
...
begin
  ...
  try
    ...
    TTitulosCliente.new.CodVend(CodVend).DeleteTitulos.BuscaTitulosServidor(DataSet).GravaTitulosSqlite(DataSet);
    TTitulosCliente.new.CodVend(CodVend).DeleteItensTitulo.BuscaItensTituloServidor(DataSet)
