unit uClasseResumoVendedorClientes;

interface

uses uInterfaceResumoVendedorClientes, FMX.ListView, uDmDados, uRESTDWPoolerDB,
...
type
  ...
  TResumoVendedorClientes = class(TInterfacedObject, iModelResumoVendedorClientes)
  private
    FCodVend: Integer;
    FPesoTot: Double;
    FVlrTot:  Double;
...
{ TResumoVendedorClientes }

class function TResumoVendedorClientes.new: iModelResumoVendedorClientes;
begin
  result := self.create;
end;

constructor TResumoVendedorClientes.create;
begin

end;

destructor TResumoVendedorClientes.destroy;
begin

  inherited;
end;

function TResumoVendedorClientes.CodVend(value: Integer): iModelResumoVendedorClientes;
begin
  result   := self;
  FCodVend := value;
end;

function TResumoVendedorClientes.QntReal(value: Integer): iModelResumoVendedorClientes;
begin
  result   := self;
  FQntReal := value;
end;

function TResumoVendedorClientes.VlrTot(value: Double): iModelResumoVendedorClientes;
begin
  result  := self;
  FVlrTot := value;
end;

function TResumoVendedorClientes.PesoTot(value: Double): iModelResumoVendedorClientes;
begin
  result   := self;
  FPesoTot := value;
end;

function TResumoVendedorClientes.LimpaTabelas: iModelResumoVendedorClientes;
var
  qry: TFDQuery;
begin
  result := self;
...
function TResumoVendedorClientes.PopulaClienteVendidoSqLite(ADataSet: TFDMemTable): iModelResumoVendedorClientes;
var
  i:   Integer;
  qry: TFDQuery;
begin
...
function TResumoVendedorClientes.PopulaListView(value: TListView; AList: TStringList): iModelResumoVendedorClientes;
var
  x:                         Integer;
  qry:                       TFDQuery;
  item:                      TListViewItem;
...
function TResumoVendedorClientes.BuscaClienteVendidosServidor(dt_ini, dt_fim: TDatetime; vend: Integer; ADataSet: TFDMemTable)
...
begin
  ...
  try
    ...
    rdwSQLTemp.SQL.Add(' select tp.cod_cliente, ' + ' isnull(sum(th.qtd_tot), 0) as qtd_tot, ' + ' isnull(sum(th.pes_tot), 0) as pes_tot, '
      ...
      ' left outer join v_prestador ts on (tp.cod_prest = ts.cod_prest) ' + ' left outer join t_centro_custo tu on (tp.cod_cc = tu.cod_cc) '
