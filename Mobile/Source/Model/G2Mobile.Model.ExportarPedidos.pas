...
uses
  ...
  FMX.Objects,
  uFrmUtilFormate,
  Form_Mensagem, G2Mobile.Controller.Pedidos;
...
type
  ...
  TModelExportPedido = class(TInterfacedObject, iModelExportarPedidos)
  private
    FQry: TFDQuery;
...
class function TModelExportPedido.new: iModelExportarPedidos;
begin
  result := self.create;
end;

constructor TModelExportPedido.create;
begin
...
destructor TModelExportPedido.destroy;
begin
  FreeAndNil(FQry);
...
function TModelExportPedido.DeleteItemAtualizado: iModelExportarPedidos;
begin
  result := self;
  FQry.ExecSQL(CONST_DELETEITEMATUALIZADO);
end;

function TModelExportPedido.VerificaPedidosSincronizadosSqlite: iModelExportarPedidos;
var
  I: Integer;
begin
  result := self;
...
function TModelExportPedido.BuscaInfoAtualizadaServidor(DataSet: TFDMemTable): iModelExportarPedidos;
begin
  result := self;
...
function TModelExportPedido.GravaInfoAtulizadoPedidoSqlite(DataSet: TFDMemTable): iModelExportarPedidos;
var
  I: Integer;
begin
  result := self;
