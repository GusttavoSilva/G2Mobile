...
function TModelClientes.LimpaTabelaClientes: iModelClientes;
var
  qry: TFDQuery;
begin
  result := self;
...
function TModelClientes.BuscaClientesServidor(Super, cod_vend: Integer; ADataSet: TFDMemTable): iModelClientes;
var
...
function TModelClientes.BuscarCliente(value: Integer): String;
var
  qry: TFDQuery;
begin
...
function TModelClientes.PopulaClientesSqLite(ADataSet: TFDMemTable): iModelClientes;
var
  i  : Integer;
  qry: TFDQuery;
begin
...
function TModelClientes.PopulaListView(value: TListView; positivo, negativo: TImage; Pesq: String): iModelClientes;
var
  x   : Integer;
  qry : TFDQuery;
  item: TListViewItem;
...
begin
  try
    ...
    qry.SQL.Add
      ...
      + 'where cod_pessoa = ' + QuotedStr(Pesq) + ' or nome_fant like ''%' + Pesq + '%'' or nome_razao like ''%' +
      Pesq + '%''');
    qry.Open;
    qry.First;
...
function TModelClientes.RetornaLimite(value: Integer): String;
var
  qry: TFDQuery;
begin
...
function TModelClientes.ValidaCliente(value: Integer): Integer;
var
  qry: TFDQuery;
begin
  ...
  if (qry.FieldByName('bloq_clientes').AsInteger = 0) and (qry.FieldByName('situacao').AsInteger <> 1) then
    ...
    if (qry.FieldByName('bloq_clientes').AsInteger = 1) and (qry.FieldByName('situacao').AsInteger <> 1) then
      result := 2 // bloquea
    else
      result := 3;
...
function TModelClientes.PopulaCampos(value: Integer; AList: TStringList): iModelClientes;
var
  i  : Integer;
  qry: TFDQuery;
begin
