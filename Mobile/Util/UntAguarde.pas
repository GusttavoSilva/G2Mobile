...
type
  TAguarde = class
    ...
    public
      ...
      class procedure Show(AParent: TFMXObject; const AMessage: string = '');
...
class procedure TAguarde.Show(AParent: TFMXObject; const AMessage: string);
begin
  FFundo                       := TPanel.Create(AParent);
  FFundo.Parent                := AParent;//AParent;
  FFundo.Visible               := True;
  ...
  FAniLoading                  := TAniIndicator.Create(AParent);
  ...
  FLabelMessage                := TLabel.Create(AParent);
